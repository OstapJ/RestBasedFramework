package com.epam.rest.report_portal.testng;
...
import com.epam.rest.report_portal.common.Statuses;
import com.epam.rest.report_portal.common.TestMethodType;
...
//
//		logger.debug(testData.prettyPrint());
		logger.debug("Test data doesn't want to print. For some reasons");
	...
	public void sendReportPortalMsg(ITestResult result) {
		...
		slrq.setLogTime(Calendar.getInstance().getTime());
		logger.debug("Im in TestNG Service class");
		...
		slrq.setFile(file);
		logger.debug("A eto poyavitsa ili net");
		...
		logger.debug("sendReportPortalMsg");
	}

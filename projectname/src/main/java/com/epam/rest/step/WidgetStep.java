...
public class WidgetStep extends AbstractStepDefinition {
    ...
    public void getWidgetNames(final DataTable table) {
        ...
        HttpResponseModel<List<String>> response = widgetService.getWidgetNames(t.getValue("projectName"));
    ...
    public void getWidget(final DataTable table) {
        BaseDataTable t = new BaseDataTable(table);
        HttpResponseModel<WidgetDTO> response = widgetService.getWidget(t.getValue("projectName"),
                t.getValue("widgetId"));
        this.responseWidget = response;
    }
